...
namespace OmniWheel
{
    public sealed class OneDriveConnector
    {
        ...
        private const string TokenContentFormatAccess = "client_id={0}&redirect_uri={1}&{3}={4}&grant_type={5}";
       ...
       /// <summary>
        ...
        private async Task<HttpResponseMessage> GetTokens(string accessCodeOrRefreshToken, string requestType, string grantType)
        {
            using (HttpRequestMessage requestMessage = new HttpRequestMessage(HttpMethod.Post, new Uri(TokenUri)))
            {
                ...
                //if (requestType == "code")
                    requestContent = string.Format(TokenContentFormatAccess, clientId, redirectUrl, clientSecret, requestType, accessCodeOrRefreshToken, grantType);
                //else
